...
class Maxxdev_Helper_Controller {
	...
	public static function initController($template) {
		...
		if (is_single() && !in_array($pagename, Maxxdev_Helper_Pages::$pages)) {
			...
			if (strlen($page) == 0) {
				$file = ABSPATH . DIRECTORY_SEPARATOR . "app/controllers/" . str_replace("_Controller", "", self::getClassNameFromPageName($pagename)) . ".php";
				if (file_exists($file)) {
					$page = $pagename;
				}
			}
		...
		if (strlen($page) > 0) {
			...
			// get filename
			$fileName = str_replace("_Controller", "", $className);
			...
			if (class_exists($className)) {
				...
				if (method_exists($controllerClass, "start")) {
					// execute $class->start();
					return $controllerClass->start($template);
				}
			}
		}
